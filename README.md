...
- **Self-tuning** — grid-searches the BM25 + vector fusion weights on a train split and adopts them only if they beat the neutral baseline on a held-out val split — held-out Recall@1 0.40 → 0.60 (*n=10 held-out probes — illustrative, not statistically significant; the point is the holdout discipline*). Committed to [`tune_result.json`](tune_result.json), like [`ab_result.json`](ab_result.json).
