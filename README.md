...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-128%2F128_green-059669?style=for-the-badge&labelColor=0b0f14" alt="128/128 tests">
...
Five distinct Qwen roles across **four Qwen models** (`qwen-max` · `qwen-plus` · `qwen-turbo` · `text-embedding-v4`) and three Qwen Cloud APIs. The **live deployment routes each role to the model that measured best for it** (`RG_MODEL_ROUTING` on, `QWEN_MODEL=qwen-max`); the flag-off reproducible baseline — the 128 tests and the A/B — collapses every role to a single `qwen-plus`, byte-identical. **Alibaba Cloud deploy proof (code):** the DashScope / Qwen Cloud client [`backend/qwen_client.py`](backend/qwen_client.py) · deployment notes [`deploy/README.md`](deploy/README.md).
...
> Tests: `pytest` (offline, 128/128) · `pytest -m live` (hits Qwen).
