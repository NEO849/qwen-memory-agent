...
And the core thesis is now **grounded live** on the deployment: a step (`harness/ground_demo.py`) writes **real** pytest outcomes into the live ledger, so confidence is *earned*, not a prior. A correct money lesson (integer cents) took **3/3 real passes → confidence 0.86 (a validated node)**; a wrong one (float dollars) took **3/3 real fails → tombstoned** (a grey "forgotten" node) — *the memory forgetting what's wrong, with real evidence*. `/metrics` now shows **`grounded_outcomes > 0`** and an honest **`calibration_gap` of 0.143** (displayed confidence vs. empirical pass-rate) instead of a misleading 0.0.
...
## How we built it
- **Qwen Cloud** in five roles: DISTILL (`qwen-plus`, JSON), RECALL (`text-embedding-v4`), REVISE (`qwen-plus`), SELF-CHECK (`qwen-plus` — paraphrase-based self-evaluation + contradiction judging), and FUNCTION-CALLING in chat (below).
...
- **Grounding the thesis live** (`harness/ground_demo.py`): writes real pytest outcomes into the live ledger (money-rounding pattern, independent of the get_orders demo) so a correct lesson earns confidence from **3/3 real passes** (0.86, validated) and a wrong one is **tombstoned on 3/3 real fails** — making `/metrics` report `grounded_outcomes > 0` and a real `calibration_gap` (0.143). Reproduce: `python -m harness.ground_demo <ledger>`.
...
- **Associative memory** (Hebbian wiring + spreading activation): lessons recalled together strengthen a capped synapse weight, and an opt-in spreading-activation recall walks the strongest synapses to surface neighbours pure retrieval misses — neuroscience-*inspired*, and deliberately kept separate from confidence (which stays test-earned). The globe now shows **66 lessons and ~200 edges** with variable synapse strength.
