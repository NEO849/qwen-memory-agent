...
## How we built it
- **Qwen Cloud** in five roles across **four Qwen models**: DISTILL (`qwen-plus`, JSON), RECALL (`text-embedding-v4`), REVISE (`qwen-max` judge), SELF-CHECK (`qwen-turbo` paraphrase + `qwen-max` contradiction judge), and FUNCTION-CALLING in chat on flagship **`qwen-max`** (below). The live deployment **routes each role to the model that measured best for it** (`/telemetry` shows it live); the reproducible A/B and 125-test baseline fix a single `qwen-plus`, byte-identical.
