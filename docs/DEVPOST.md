...
The memory also carries **anti-patterns** — dead-end rules a past regression proved wrong — injected as active ⛔ DO NOT inhibitions rather than guidance, so the agent is steered *away* from a known bad path, not just toward a good one. It can also crystallize a cluster of related lessons into one higher-level meta-lesson (ExpeL-style synthesis) that then earns its own confidence from real tests like any other.
