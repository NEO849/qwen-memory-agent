...
2. **60% of the score = Innovation + Technical Depth** → we *say out loud*: Qwen in 5 roles across 4 models (qwen-max flagship, routed live), MCP, "custom algorithm", "stateless / scales out", "error handling", 128 tests.
...
- **Technical Depth 30%** (Qwen-API depth + algorithmics) → [0:32] Qwen function-calling · 5 roles / 4 models · [0:52] custom confidence algorithm + hybrid retrieval + atomic/no-race · [1:48] MCP · 128 tests + red-team.
