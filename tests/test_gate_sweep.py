...
def test_wide_clean_band_and_0_62_robust(monkeypatch):
    ...
    assert r["correct_confidences"] == [0.875] * 5          # 6/6 pass → Beta(7,1)
    assert r["poison_confidences"] == [0.125] * 5           # 0/6 fail → Beta(1,7)
    ...
    # at a clean gate: all 5 correct injected, 0 poison
    clean = [s for s in r["sweep"] if s["gate"] == 0.62][0]
    assert clean["correct_injected"] == 5 and clean["poison_injected"] == 0
